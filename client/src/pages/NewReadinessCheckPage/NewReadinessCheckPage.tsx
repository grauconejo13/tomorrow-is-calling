...
export function NewReadinessCheckPage({ initial, onReview, onCancel }: { initial: TransportRequestForm; onReview: (data: TransportRequestForm) => void; onCancel: () => void }) {
  ...
  return <main className="page form-page"><p className="eyebrow">Customer transport form</p><h1>Complete your transport details</h1><p className="page-intro">Prototype data only. These details are used to prepare the transport quote and CALL-E customer decision call.</p>{error && <div className="form-error" role="alert">{error}</div>}<form onSubmit={submit} noValidate>
    <section><p className="section-label">Customer</p><div className="form-grid"><label>Full name<input value={data.customer.fullName} onChange={(e) => customer("fullName", e.target.value)} required /></label><label>Phone<input type="tel" value={data.customer.phone} onChange={(e) => customer("phone", e.target.value)} required /></label><label>Email<input type="email" value={data.customer.email} onChange={(e) => customer("email", e.target.value)} required /></label></div></section>
    ...
    <section><p className="section-label">Service authorization</p><label>Special instructions<textarea rows={4} value={data.specialInstructions} onChange={(e) => update("specialInstructions", e.target.value)} /></label><div className="prototype-notice">Payment happens only after the customer accepts the quote. A secure checkout link is then sent by email or text. Do not enter card or bank information in this transport form.</div></section>
    <label className="consent"><input ref={consentRef} type="checkbox" checked={data.consentToContact} onChange={(e) => update("consentToContact", e.target.checked)} /> I consent to service-related calls and texts about this transport request.</label><div className="form-actions"><button className="button button--primary" type="submit">Review quote call</button><button className="button button--quiet" type="button" onClick={onCancel}>Cancel</button></div></form></main>;
}
