...
export function NewReadinessCheckPage({ initial, onReview, onCancel }: { initial: TransportRequestForm; onReview: (data: TransportRequestForm) => void; onCancel: () => void }) {
  ...
  return <main className="page form-page"><p className="eyebrow">Customer transport form</p><h1>Complete your transport details</h1><p className="page-intro">Prototype data only. In production, this secure link is sent after a representative creates the request.</p>{error && <div className="form-error" role="alert">{error}</div>}<form onSubmit={submit} noValidate>
    ...
    <section><p className="section-label">Service authorization</p><label>Special instructions<textarea rows={4} value={data.specialInstructions} onChange={(e) => update("specialInstructions", e.target.value)} /></label><div className="prototype-notice">Secure payment and service authorization would be completed through a secure external channel in production. Do not enter a credit-card number here.</div></section>
    <label className="consent"><input ref={consentRef} type="checkbox" checked={data.consentToContact} onChange={(e) => update("consentToContact", e.target.checked)} /> I consent to service-related calls and texts about this transport request.</label><div className="form-actions"><button className="button button--primary" type="submit">Review follow-up call</button><button className="button button--quiet" type="button" onClick={onCancel}>Cancel</button></div></form></main>;
}
