...
type Props = {
  initial: TransportRequestForm;
  onChange: (data: TransportRequestForm) => void;
  onReview: (data: TransportRequestForm) => void;
  onCancel: () => void;
};
...
export function NewReadinessCheckPage({ initial, onChange, onReview, onCancel }: Props) {
  ...
  const ContactFields = ({ kind }: { kind: "pickup" | "delivery" }) =>
    data[kind].presence === "alternate" ? (
      <div className="form-grid alternate-fields">
        <label>
          Authorized {kind} contact name
          <input value={data[kind].alternateContact?.fullName ?? ""} onChange={(e) => alternate(kind, "fullName", e.target.value)} required />
        </label>
        <label>
          Mobile phone
          <input type="tel" value={data[kind].alternateContact?.phone ?? ""} onChange={(e) => alternate(kind, "phone", e.target.value)} required />
        </label>
        <label>
  ...
  return (
    <main className="page form-page">
      <p className="eyebrow">Customer transport form</p>
      ...
      <form onSubmit={submit} noValidate>
        ...
        {(["pickup", "delivery"] as const).map((kind) => (
          <section key={kind}>
            ...
            <fieldset>
              <legend>Who will be present?</legend>
