...
function reserveMemory(key, limit, windowSeconds) {
  ...
  if (next.count > limit) {
    const retryAfterSeconds = Math.max(1, Math.ceil((next.expiresAt - now) / 1000));
    return { allowed: false, count: next.count - 1, retryAfterSeconds };
  }
...
export function createDemoRateLimiter(env = process.env) {
  ...
  async function reserveKey(key, limit) {
    if (useRedis) {
      return reserveRedis({ url: redisUrl, token: redisToken, key, limit, windowSeconds });
    }
    return reserveMemory(key, limit, windowSeconds);
  }

  async function releaseKey(key) {
    if (useRedis) {
      return releaseRedis({ url: redisUrl, token: redisToken, key });
    }
    return releaseMemory(key);
  }
